...
REQUIRES: DDPM checkpoint at the path specified in DDPM_CKPT.
This is the ENHANCED method with FID-based quality weighting.

Kaggle Account #3

IMPORTANT: Paste the contents of ddim_sampler.py into a CELL ABOVE this one first!
"""
...
from ddim_sampler import load_ddpm_model, ddim_generate
...
# ── FID Quality Scores (from per_class_fid.py with 100 DDIM steps) ──
FID_SCORES = {
    0: 40.6128,  # airplane
...
# Higher weight = better quality = more synthetic data trusted
# Formula: weight = max(0.1, 1.0 - (fid - 25) / 50)
...
# ═══════════════════════════════════════════════════════════════════════════════
# SERVER-SIDE KNOWLEDGE DISTILLATION WITH FID FILTERING (Core Contribution)
# ═══════════════════════════════════════════════════════════════════════════════
...
def server_knowledge_distillation(
    ...
    FID weights are used to down-weight classes with poor generation quality.
    """
    ...
    # Class generation weights — modulated by FID quality
    gen_w = total.copy()
    if fid_weights:
        for c in range(num_classes):
    ...
    for step in range(kd_cfg['kd_iterations']):
        # Generate labels proportional to class distribution (weighted by FID)
        ...
        # Apply FID quality weighting to soft labels
        if fid_weights:
...
if __name__ == '__main__':
    ...
    EXP_NAME = 'E3_FedAvg_KD_FID'

    print(f"\n{'='*60}")
    print(f"EXPERIMENT: {EXP_NAME}")
    print(f"  KD: ON")
    print(f"  FID filtering: ON")
    print(f"{'='*60}")
    ...
    for rnd in range(start_round, FL_CFG['global_rounds']):
        ...
        # Server-side Knowledge Distillation WITH FID FILTERING
        avg_model, kd_losses = server_knowledge_distillation(
            avg_model, client_params, selected, client_class_counts,
            model_template, ddpm_model, ddpm_scheduler, ddim_generate,
    ...
    with open(RESULTS_DIR / 'fid_scores.json', 'w') as f:
        json.dump({'fid_scores': {CLASS_NAMES[c]: v for c, v in FID_SCORES.items()},
                   'fid_weights': {CLASS_NAMES[c]: v for c, v in FID_WEIGHTS.items()}}, f, indent=2)
